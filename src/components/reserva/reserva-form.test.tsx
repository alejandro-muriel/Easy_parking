...
describe('ReservaForm', () => {
  ...
  it('carga horarios y plazas al montar', async () => {
    ...
    expect(globalThis.fetch).toHaveBeenCalledWith('/api/plazas');
  });
  ...
  it('llama a fetch para enviar reserva', async () => {
    ...
    vi.mocked(globalThis.fetch).mockImplementation((url) => {
      if (url === '/api/horarios' || url === '/api/plazas') {
        return Promise.resolve({
          ok: true,
  ...
  it('permite entrar a cola para una plaza no disponible', async () => {
    const mockHorarios = [
      {
        id: '1',
        materia: 'Matemáticas',
    ...
    vi.mocked(globalThis.fetch).mockImplementation((url, options) => {
      ...
      if (url === '/api/plazas/2/cola' && (!options || !options.method)) {
        colaGetCount += 1;
        return Promise.resolve({
          ok: true,
          json: () => Promise.resolve({
            cola:
              colaGetCount > 1
                ? [{ id: 'q1', idUsuario: '1', idPlaza: '2', estado: 'ACTIVA', fechaRegistro: new Date().toISOString(), usuario: { id: '1', name: 'Test User', email: 'test@poli.edu.co' } }]
                : [],
          }),
        } as Response);
      }

      if (url === '/api/plazas/2/cola' && options?.method === 'POST') {
        return Promise.resolve({
          ok: true,
          json: () => Promise.resolve({ message: 'Ingresaste a la cola de espera.' }),
        } as Response);
      }
    ...
    render(<ReservaForm user={{ ...mockUser, role: { name: 'ESTUDIANTE' } }} />);

    await waitFor(() => {
      expect(screen.getByRole('button', { name: 'Entrar en cola' })).toBeInTheDocument();
    });

    fireEvent.click(screen.getByRole('button', { name: 'Entrar en cola' }));

    await waitFor(() => {
      expect(screen.getByRole('button', { name: 'Salir de cola' })).toBeInTheDocument();
      expect(screen.getByText(/Ingresaste a la cola de espera/i)).toBeInTheDocument();
    });
  });
});